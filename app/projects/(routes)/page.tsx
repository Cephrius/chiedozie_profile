...
const Project = ({ title, description, image, link }: ProjectProps) => (
  <li className="flex gap-4 items-center transition-opacity ">
    <a
      ...
      target="_blank"
    >
      <Image src={image} alt={title} className="object-center " fill />
    </a>
    <div className="grow flex justify-between gap-2 items-center">
      <div className="space-y-1 flex-1">
        {" "}
        {/* Added flex-1 to occupy remaining space */}
        <h3 className="text-primary line-clamp-2 leading-tight font-medium mt-[0.5rem]">
          {" "}
          {/* Added mt-[0.5rem] to adjust margin */}
          {title}
        </h3>
...
const listProject = [
  ...
  },
  {
    ...
    description:
      "a REST API (Representational State Transfer Application Programming Interface) is an architectural style for designing networked applications. It stands for Representational State Transfer and is based on the principles of using standard HTTP methods (such as GET, POST, PUT, DELETE) for communication between clients and servers.",
    image: "/image.png",
